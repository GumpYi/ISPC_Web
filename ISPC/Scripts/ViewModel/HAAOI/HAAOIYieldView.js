...
var YieldViewModel = function () {
    ...
    self.loadYieldViewData = function () {
        ...
        $.ajax({
            url: "/HAAOIYieldView/GetYieldViewData",
            type: "POST",
            ...
            success: function (data) {   
                ...
                $.each(data.DPMOBySelectedSection, function (key, value) {
                ...
                // art the fppm chart
                var chartfppm = $("#FPPMContainer").highcharts();
                var series = chartfppm.series;
                while (series.length > 0) {
                    series[0].remove(false);
                }
                chartfppm.redraw();
                var fppmcategories = new Array();
                var fppmdata = new Array();
                $.each(data.FPPMBySelectedSection, function (key, value) {
                    fppmcategories.push(key);
                    fppmdata.push(value);
                });
                chartfppm.addSeries({
                    data: fppmdata
                }, false);
                chartfppm.xAxis[0].setCategories(fppmcategories, false);
                chartfppm.redraw();
                spinner.spin();
            },
